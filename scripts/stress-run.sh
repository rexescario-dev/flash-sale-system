...
Supported scenarios for k6 run: harness-smoke, purchase-load, oversell.
EOF
      exit 0
      ;;
    *)
...
case "$SCENARIO" in
  ...
  oversell)
    ...
    ;;
  *)
    echo "error: scenario '$SCENARIO' is not implemented for k6 yet (supported: harness-smoke, purchase-load, oversell)" >&2
    exit 1
    ;;
esac
